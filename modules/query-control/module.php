<?php
namespace EpicJungleElementor\Modules\QueryControl;

use Elementor\Controls_Manager;
use Elementor\Core\Common\Modules\Ajax\Module as Ajax;
use Elementor\Widget_Base;
use EpicJungleElementor\Base\Module_Base;
use EpicJungleElementor\Modules\QueryControl\Controls\Group_Control_Posts;
use EpicJungleElementor\Modules\QueryControl\Controls\Group_Control_Query;
use EpicJungleElementor\Modules\QueryControl\Controls\Group_Control_Related;
use EpicJungleElementor\Modules\QueryControl\Classes\Elementor_Post_Query;
use EpicJungleElementor\Modules\QueryControl\Classes\Elementor_Related_Query;
use EpicJungleElementor\Modules\QueryControl\Controls\Query;
...
class Module extends Module_Base {
	...
	public static function add_exclude_controls( $widget ) {
		...
		$widget->add_control(
			'exclude',
			[
				'label' => __( 'Excluir', 'epicjungle-elementor' ),
				'type' => Controls_Manager::SELECT2,
				'multiple' => true,
				'options' => [
					'current_post' => __( 'Postagem atual', 'epicjungle-elementor' ),
					'manual_selection' => __( 'Seleção manual', 'epicjungle-elementor' ),
				],
				'label_block' => true,
			]
		);
		...
		$widget->add_control(
			'exclude_ids',
			[
				'label' => __( 'Pesquisar e selecionar', 'epicjungle-elementor' ),
		...
		$widget->add_control(
			'avoid_duplicates',
			[
				'label' => __( 'Evite duplicatas', 'epicjungle-elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
				'description' => __( 'Defina como Sim para evitar que postagens duplicadas apareçam na página. Isso afeta apenas o frontend.', 'epicjungle-elementor' ),
			]
		);
	...
	public function ajax_posts_filter_autocomplete_deprecated( $data ) {
		...
		switch ( $data['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters_deprecated( 'epicjungle_elementor/query_control/get_autocomplete/' . $data['filter_type'], $data, '3.0.0', 'elementor/query/get_autocomplete/' . $data['filter_type'] );
	...
	public function ajax_posts_filter_autocomplete( array $data ) {
		...
		switch ( $query_data['object'] ) {
			...
			case self::QUERY_OBJECT_LIBRARY_TEMPLATE:
				...
				foreach ( $query->posts as $post ) {
					$document = EpicJungleElementor\Plugin::elementor()->documents->get( $post->ID );
					if ( $document ) {
	...
	public function ajax_posts_control_value_titles_deprecated( $request ) {
		...
		switch ( $request['filter_type'] ) {
			...
			case 'author':
				...
				break;
			default:
				$results = apply_filters_deprecated( 'epicjungle_elementor/query_control/get_value_titles/' . $request['filter_type'], $request, '3.0.0', 'elementor/query/get_value_titles/' . $request['filter_type'] );
	...
	public function register_controls() {
		$controls_manager = EpicJungleElementor\Plugin::elementor()->controls_manager;
	...
	/**
	 * @param \EpicJungleElementor\Base\Base_Widget $widget
	 * @param string $name
	...
	public function localize_settings( $settings ) {
		$settings = array_replace_recursive( $settings, [
			'i18n' => [
				'all' => __( 'Tudo', 'epicjungle-elementor' ),
			],
		] );
